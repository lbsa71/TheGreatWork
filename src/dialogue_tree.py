...
class DialogueTree:
    ...
    def _fix_node_structure(self) -> None:
        ...
        for node_id, node_data in self.nodes.items():
            ...
            if "situation" not in node_data or not isinstance(node_data["situation"], str):
                nodes_to_nullify.append(node_id)
                continue
    ...
    def _extract_and_add_missing_params(self) -> None:
        ...
        for param_name in all_effect_params:
            if param_name not in self.params:
                logger.info(f"Adding missing parameter '{param_name}' to root params with default value 0")
