...
class TestDialogueWebApp(unittest.TestCase):
        ...
        with web_ui.app.test_client() as client:
            ...
            update_data = {
                ...
                "choices": [
                    ...
                    {"text": "Updated choice 2", "next": "node2"},
                ],
            }

            response = client.put(
                "/api/node/start", json=update_data, content_type="application/json"
            )
            self.assertEqual(response.status_code, 200)
            data = response.get_json()
            ...
            self.assertEqual(
                web_ui.tree.nodes["start"]["situation"], "Updated situation text"
            )
            self.assertEqual(len(web_ui.tree.nodes["start"]["choices"]), 2)
            self.assertEqual(
                web_ui.tree.nodes["start"]["choices"][0]["text"], "Updated choice 1"
            )
            self.assertEqual(
                web_ui.tree.nodes["start"]["choices"][0]["effects"]["param1"], 10
            )
    ...
    def test_update_node_validation(self, mock_node_gen: Any, mock_ollama: Any) -> None:
        ...
        with web_ui.app.test_client() as client:
            ...
            response = client.put(
                "/api/node/start", json={"choices": []}, content_type="application/json"
            )
